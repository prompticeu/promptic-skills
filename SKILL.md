---
name: promptic
description: Integrate the Promptic Python SDK for LLM observability, tracing, prompt optimization, datasets, and prompt evaluators. Use when code imports `promptic_sdk`, user asks to add Promptic tracing, optimize prompts, deploy prompts, or integrate with the Promptic platform. Also use when setting up OpenTelemetry-based LLM tracing, architecture tracing, workflow tracing, semantic parent/child spans, canonical datasets, or managing AI components programmatically.
---

# Promptic Python SDK

SDK and CLI for the [Promptic](https://promptic.eu) platform — LLM tracing and prompt optimization.
...
promptic datasets get <ds-id> --component <id>   # Get dataset with cases
