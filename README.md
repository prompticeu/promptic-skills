# Promptic Skills

AI agent skills for integrating with the [Promptic](https://promptic.eu) platform.

## Install

```bash
npx skills add prompticeu/promptic-skills
```

## Skills

| Skill | Description |
|-------|-------------|
| **promptic** | LLM tracing, prompt optimization, agent evaluation, and external Agent Gym leaderboard submissions with the Promptic Python SDK |

## What are skills?

Skills are reusable capabilities for AI coding agents (Claude Code, Cursor, Windsurf, etc.). They provide procedural knowledge that helps agents accomplish specific tasks more effectively.

Learn more at [skills.sh](https://skills.sh).

## License

MIT
