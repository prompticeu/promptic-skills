...
`AgentEvaluation` status: `"pending" | "running" | "completed" | "failed"`. Each evaluation has a `subject` (`"output" | "trajectory" | "annotation"`) and a `targetType` (`"trace" | "run" | "dataset"`); the anchor IDs match the target type — `trace` populates only `traceDbId`, `dataset` only `datasetId`, and `run` populates both `runId` and `datasetId` (a run belongs to a dataset). A nullable `budget` field carries optional per-evaluation budget configuration and is `null` unless one was set. The `results` field still contains the legacy `InsightResult` blob (`insights`, `judgeResults`, `meta`) for dataset/run evaluations; new code should prefer `list_judge_results(...)` which returns first-class `JudgeResult` rows with `judgeKey`, `judgeName`, `backend`, the target FKs (`datasetItemId` / `traceDbId` / `annotationId` / `runId`), `score`, `rationale`, `evidence`, `analysisPayload`, an immutable `judgeSnapshot`, and a `version` that increments per re-run. See the example in `SKILL.md` for iteration patterns.
